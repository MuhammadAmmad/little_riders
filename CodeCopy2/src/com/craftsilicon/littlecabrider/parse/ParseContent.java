		...
		try {
			...
			if (jsonObject.getBoolean(KEY_SUCCESS)) {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					type.setIconSelected(typeJson.getString(ICON));
					type.setId(typeJson.getInt(ID));
					type.setName(typeJson.getString(NAME));
					type.setPricePerUnitDistance(typeJson.getDouble(PRICE_PER_UNIT_DISTANCE));
					type.setPricePerUnitTime(typeJson.getDouble(PRICE_PER_UNIT_TIME));
