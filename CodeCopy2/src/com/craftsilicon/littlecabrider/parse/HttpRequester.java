...
public class HttpRequester {
	...
	public HttpRequester(Activity activity, Map<String, String> map,
		...
		if (AndyUtils.isNetworkAvailable(activity)) {
			...
			// .get("url"));
			request = (AsyncHttpRequest) new AsyncHttpRequest() .executeOnExecutor(Executors.newSingleThreadExecutor(), map.get("url"));
		} else {
			showToast("Network is not available!!!");
		}
	}

	public HttpRequester(Activity activity, Map<String, String> map,int serviceCode, boolean isGetMethod,
	...
	class AsyncHttpRequest extends AsyncTask<String, Void, String> {
			...
			try {
				...
				if (!isGetMethod) {
					...
					for (String key : map.keySet()) {
						//AppLog.Log(Const.TAG, key + ":" + map.get(key));
		...
		protected void onPostExecute(String response) {
			...
			AndyUtils.log("allresponsestag:all", ""+response);	
