...
import com.craftsilicon.littlecabrider.adapter.PaymentListAdapter;
...
import com.craftsilicon.littlecabrider.utils.Sc;
...
// implements PWTokenObtainedListener, PWTransactionListener
{
	...
	private ArrayList<com.craftsilicon.littlecabrider.models.Card> listCards;
	...
	EditText edtRegisterCreditCardName, etCreditCardNum, etCvc, etYear, etMonth;
	MyFontButton txtSkip;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		txtSkip = (MyFontButton) view.findViewById(R.id.txtSkip);
		btnAddNewPayment = (LinearLayout) view.findViewById(R.id.tvAddNewPayment);
		// btnScan = (LinearLayout) view.findViewById(R.id.btnScan);
		btnAddNewPayment.setOnClickListener(this);
		txtSkip.setOnClickListener(this);
		listCards = new ArrayList<com.craftsilicon.littlecabrider.models.Card>();
		adapter = new PaymentListAdapter(activity, listCards, new PreferenceHelper(activity).getDefaultCard());
	...
	protected boolean isValidate() {		
		if (edtRegisterCreditCardName.getText().length() == 0
				||
				etCreditCardNum.getText().length() == 0
	...
	public void onTaskCompleted(String response, int serviceCode) {
		...
		case Const.ServiceCode.ADD_CARD:
			...
			AppLog.Log("addpayment_fragmentregister", "datareturned:" + response);
			if (new ParseContent(activity).isSuccess(response)) {
				AndyUtils.showToast(getString(R.string.text_add_card_scucess), activity);
				activity.setResult(Activity.RESULT_OK);
			} else {
				AndyUtils.showToast(getString(R.string.text_not_add_card_unscucess), activity);
	...
	private void addCard(String stripeToken, String lastFour) {	
		...
		sb.append("CARDNUM|"+etCreditCardNum.getText().toString().trim().replaceAll("\\s+|-", "")+"|");
		//sb.append("CARDCVV|"+etCvc.getText().toString()+"|");
		sb.append("CARDMONTH|"+etMonth.getText().toString()+"|");
		sb.append("CARDYEAR|"+etYear.getText().toString()+"|");	
		sb.append("CODEBASE|"+"ANDROID|");
		sb.append("IMEI|"+new PreferenceHelper(getActivity()).getIMEI()+"|");
		/*String randomkey 	 = Sc.DecryptData(Const.RK, Sc.DecryptData(Const.PK, "CHATAPP7"));
		String datatoencrypt = sb.toString();
		datatoencrypt		 = Sc.EncryptData(datatoencrypt, randomkey);
		map.put("c2", sb.toString());
		map.put("c1", datatoencrypt);*/
		String encryptedc1 = PreferenceHelper.eaes(sb.toString());
		map.put("c2", sb.toString());
		map.put("c1", encryptedc1);

		AppLog.Log("addpayment_activity", "datasent:" + sb.toString());
		AppLog.Log("addpayment_activity", "datasentencrypted:" + encryptedc1);
		AppLog.Log("addpayment_activity", "datasentdecrypted:" + PreferenceHelper.daes(encryptedc1));
	...
	public void showAddCardDialog() {
		...
		addCardDialog.getWindow().setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
		addCardDialog.setContentView(R.layout.fragment_payment);

		btnAddPayment = (ImageView) addCardDialog.findViewById(R.id.btnAddPayment);
		etCreditCardNum = (EditText) addCardDialog.findViewById(R.id.edtRegisterCreditCardNumber);
		edtRegisterCreditCardName = (EditText) addCardDialog.findViewById(R.id.edtRegisterCreditCardName);
		...
		etMonth = (EditText) addCardDialog.findViewById(R.id.edtRegisterexpMonth);
	...
	public void saveCreditCard() {
		...
		Card card = new Card(etCreditCardNum.getText().toString(),
				...
				Integer.parseInt(etYear.getText().toString()), etCvc.getText().toString());
		...
		if (validation) {
			...
			// handleError("");
			AndyUtils.showToast("The card details that you entered are invalid", activity);
		}
	}
